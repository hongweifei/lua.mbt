...
* **`collectgarbage("step")` 与 `count` 的数字是本实现自己的。** 参考实现的 `step` 报告它所处的*相位机*是否恰好跑完一轮——空状态上 `step 0` 答 `true`，回收过大堆之后答 `false`，这里对 `size == 0` 一律答 `true`（有界、不会空转；一次完整尝试移植它的相位机，见工作清单）。`count` 报告收集器测出的活集字节，现在也把**自上一轮以来库函数创建的对象**（表格、闭包、线程、文件 userdata）计入，因此随分配上升、随回收下降，与参考实现的行为一致；绝对数字仍不同，因为它的图是分配器自己的账。
